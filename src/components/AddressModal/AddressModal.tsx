...
import { getCoordsByString, getCityPolygons, getAddressByCoords } from "../../endpoint-service/services/addressService";
...
export default function AddressModal({ isOpen, onClose, onSave, tempAuthToken, isRegistrationMode = false }: any) {
    const { city, selectCity } = locationStore;
    ...
    useEffect(() => {
        if (isManualInput && serviceToken && debouncedAddressString && debouncedAddressString.length > 3 && city?.id) {
            ...
            getCoordsByString(debouncedAddressString, city.id).then(result => {
                if (result) {
    ...
    const handleMapSelect = useCallback(async ({ coords: mapCoords }) => {
        ...
        const result = await getAddressByCoords(latitude, longitude);
            ...
            <div className="modal-container">
                ...
                <div className="modal-form">
                    ...
                    <div className="form-fields">
                        ...
                        <div className="form-group">
                            <input id="comment" type="text" placeholder=" " value={comment} onChange={(e) => setComment(e.target.value)} className="form-input" />
                        ...
                        <button onClick={handleSubmit} className="btn-submit" disabled={isLoading || !coords}>
                            {isLoading ? 'Загрузка...' : 'Выбрать этот адрес'}
                        </button>
                    </div>
                </div>
            </div>
