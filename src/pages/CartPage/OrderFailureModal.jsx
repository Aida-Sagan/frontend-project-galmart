...
const OrderFailureModal = ({ isOpen, onClose, onGoToOrders, errorMessage = "Произошла ошибка при оплате заказа. Пожалуйста, попробуйте снова. Заказ будет передан на сборку после оплаты" }) => {
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content order-failure-modal" onClick={e => e.stopPropagation()}>
                ...
                <button className="modal-btn primary-action" onClick={onGoToOrders}>
                    К заказам
                </button>
            </div>
        </div>
