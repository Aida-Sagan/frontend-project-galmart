...
import DeliveryTimeModal from '../../../pages/CartPage/DeliveryTimeModal/DeliveryTimeModal';
import PaymentMethodModal from '../../../pages/CartPage/PaymentMethodModal/PaymentMethodModal';
import ReturnOrderModal from './ReturnOrderModal';
import OrderReview from './OrderReview';
import OrderFailureModal from '../../CartPage/OrderFailureModal';
import { getOrderDetails, changeOrder, sendOrderReview } from '../../../endpoint-service/services/ordersService';
import { cartStore } from '../../../stores/cartStore';
import ManagerChatContent from '../ManagerChat/ManagerChatContent';
...
const OrderDetails = ({ order, config, onBack }) => {
    ...
    const renderActionButtons = () => {
        ...
        if (status === 'Не оплачен') {
            return (
                <>
                    <Button variant="filled" fullWidth onClick={handlePayment}>Оплатить заказ</Button>
                    <Button variant="outlined" fullWidth onClick={() => setIsCancelConfirmOpen(true)}>Отменить заказ</Button>
                </>
            );
        }
        ...
        if (status === 'Оформлен') {
            return (
                <Button variant="outlined" fullWidth onClick={() => setIsCancelConfirmOpen(true)}>
                    Отменить заказ
                </Button>
            );
        }
        ...
        if (status === 'Ожидает оценки') {
            return (
                <div style={{ display: 'flex', gap: '12px', width: '100%', flexDirection: 'column' }}>
                    <Button variant="outlined" fullWidth onClick={() => setIsReturnModalOpen(true)}>
                        Возврат/замену
                    </Button>
                    <Button variant="filled" fullWidth onClick={() => setIsShowReview(true)}>
                        Оценить заказ
                    </Button>
                </div>
            );
                ...
                <div className="modal-overlay-cancel">
                    <div className="cancel-confirm-modal">
                        ...
                        <div className="cancel-modal-actions">
                            <Button variant="filled" fullWidth onClick={() => setIsCancelConfirmOpen(false)}>Нет, вернуться к оплате</Button>
                            <Button variant="outlined" fullWidth onClick={handleCancelOrder}>Да, отменить заказ</Button>
                        </div>
                    </div>
                </div>
            )}
            {isLocationOpen && <LocationModal onClose={() => setIsLocationOpen(false)} onCitySelect={() => setIsLocationOpen(false)} />}
            {isTimeOpen && <DeliveryTimeModal isOpen={isTimeOpen} onClose={() => setIsTimeOpen(false)} onSaveSuccess={() => setIsTimeOpen(false)} />}
            {isPaymentOpen && <PaymentMethodModal isOpen={isPaymentOpen} onClose={() => setIsPaymentOpen(false)} selectedMethodId={null} onSelect={() => {}} cards={[]} onDeleteCard={() => {}} onAddCard={() => {}} />}
            {isReturnModalOpen && (
                <ReturnOrderModal
                    orderNumber={displayData.order_number}
